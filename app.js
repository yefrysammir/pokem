...
const MAPS={
  normal:{id:'normal',name:'Ruta Normal',col:'#a8a77a',bg:'linear-gradient(135deg,#d4c89a,#b8a870)',gc:'rgba(168,167,122,.35)',pool:[52,53,25,143],leg:[],legUn:false},
  fire:{id:'fire',name:'Montaña Volcánica',col:'#ee8130',bg:'linear-gradient(135deg,#ff6b35,#c0392b)',gc:'rgba(238,129,48,.35)',pool:[4,5,6],leg:[146],legUn:false},
  water:{id:'water',name:'Archipiélago Celeste',col:'#6390f0',bg:'linear-gradient(135deg,#1a6fa8,#0d4f7c)',gc:'rgba(99,144,240,.35)',pool:[7,8,9,131],leg:[144],legUn:false},
  grass:{id:'grass',name:'Bosque Esmeralda',col:'#7ac74c',bg:'linear-gradient(135deg,#2ecc71,#1a7a40)',gc:'rgba(122,199,76,.35)',pool:[1,2,3],leg:[],legUn:false},
  electric:{id:'electric',name:'Torre Eléctrica',col:'#f7d02c',bg:'linear-gradient(135deg,#f1c40f,#b7950b)',gc:'rgba(247,208,44,.35)',pool:[25,26],leg:[145],legUn:false},
  ghost:{id:'ghost',name:'Torre Lavanda',col:'#735797',bg:'linear-gradient(135deg,#2c1654,#4a235a)',gc:'rgba(115,87,151,.35)',pool:[94],leg:[],legUn:false},
  psychic:{id:'psychic',name:'Ruinas Antiguas',col:'#f95587',bg:'linear-gradient(135deg,#9b59b6,#6c3483)',gc:'rgba(249,85,135,.35)',pool:[94,149],leg:[150,151],legUn:false}
};
...
const RARITY_CFG={común:{label:'Común',col:'#95a5a6'},pseudo:{label:'Pseudo',col:'#2980b9'},especial:{label:'Especial',col:'#8e44ad'},legendario:{label:'Legendario',col:'#d4ac0d'},singular:{label:'Singular',col:'#e74c3c'}};
const CODES={
...
// ============================================================
const GS={
  player:{name:'Entrenador',gold:1000,team:[],pc:[],bag:{},usedCodes:new Set(),defeatedTrainers:new Set(),expShareOn:false,avatar:'',steps:0},
  map:{id:'normal',x:0,y:0},
  spawns:[],battle:null,screen:'radar',adventureStarted:false,viewMode:'detailed',pcPage:0,pcSearch:''
};
const MAP_LIMIT=99;
...
function save(){
  ...
  try{
    ...
    if(GS.battle&&!GS.battle.over){try{d.savedBattle=JSON.parse(JSON.stringify(GS.battle));}catch(e){}}
...
function load(){
  try{
    ...
    if(typeof GS.player.deaths!=='number')GS.player.deaths=0;
...
// ============================================================
let _nt=null;
// Prevent map keys from firing when typing in inputs
function stopMapKeys(e){e.stopPropagation();}
// Only allow letters (including accented) and numbers in inputs
...
function mkPkm(pid,variant='normal',lv=5){
  ...
  return{uid:Math.random().toString(36).slice(2,9),pid,name:b.n,types:[...b.t],variant,lv,
    ...
    capturedAt:Date.now(),captureSource:'capture',_pendingEvoItemId:null};
}
function statUp(pkm){
...
function doEvolve(uid){
  ...
  const _oldname=(pkm.nickname||pkm.name);pkm.pid=pkm.evoPending;pkm.name=tb.n;pkm.types=[...tb.t];pkm.moves=[...tb.mv];pkm.rar=tb.rar;pkm.evoPending=null;statUp(pkm);
  document.getElementById('evo-ov').style.display='none';
  notif('¡'+_oldname+' evolucionó a '+pkm.name+'!','ok',5000);save();renderCurrent();
}
...
function showSearchablePicker(title,items){
  const html=items.map(it=>`<div class="heal-pick-item picker-row" data-search="${(it.name||'').toLowerCase()}" onclick="${it.onclick}">
    ${it.sp?`<img src="${it.sp}" style="width:36px;height:36px;image-rendering:pixelated;object-fit:contain;flex-shrink:0" onerror="this.style.display='none'">` :''}
  ...
  showOv('enc-ov',`<div class="obox">
    ...
    <div style="display:flex;align-items:center;gap:6px;background:var(--card2);border:1px solid var(--brd);border-radius:8px;padding:6px 10px;width:100%">
      <span class="ms" style="color:var(--dim);font-size:18px">search</span>
      <input class="picker-search-in" type="text" placeholder="Buscar..." onkeydown="stopMapKeys(event)" onfocus="stopMapKeys(event)" oninput="filterPicker(this.value)">
    </div>
    <div id="picker-items" style="width:100%;display:flex;flex-direction:column;gap:6px;max-height:50vh;overflow-y:auto">${html||'<p style="color:var(--dim);text-align:center">Sin opciones disponibles.</p>'}</div>
    <button class="ebtn ebtn-flee" onclick="closeOv('enc-ov')"><span class="ms">close</span>Cancelar</button>
  </div>`);
}
function filterPicker(q){const r=(q||'').toLowerCase();document.querySelectorAll('#picker-items .picker-row').forEach(el=>{el.style.display=(r&&!el.dataset.search.includes(r))?'none':'flex';});}
...
function resizeRadarCanvas(){
  ...
  const h=container.clientHeight||(window.innerHeight-104);
...
function renderRadarCanvas(){
  ...
  const bgUrl=m.bgImg;
  if(bgUrl&&_sprCache[bgUrl]){
    const bgImg=_sprCache[bgUrl];
    const imgW=bgImg.naturalWidth||360;const imgH=bgImg.naturalHeight||240;
    const ox=(((GS.map.x*CELL)%imgW)+imgW)%imgW;
    const oy=(((GS.map.y*CELL)%imgH)+imgH)%imgH;
    ctx.save();ctx.globalAlpha=0.9;
    for(let tx=-ox;tx<W+imgW;tx+=imgW){for(let ty=-oy;ty<H+imgH;ty+=imgH){ctx.drawImage(bgImg,tx,ty,imgW,imgH);}}
    ctx.restore();
    ctx.save();ctx.fillStyle=m.col+'15';ctx.fillRect(0,0,W,H);ctx.restore();
  }else{
    if(bgUrl&&!_sprCache[bgUrl]){loadSpr(bgUrl,()=>renderRadarCanvas());}
  ...
  blocks.forEach(key=>{
    ...
    ctx.save();
    ctx.fillStyle='rgba(80,80,130,0.55)';ctx.fillRect(sx,sy,CELL,CELL);
    ctx.strokeStyle='rgba(160,160,220,0.7)';ctx.lineWidth=1.5;ctx.strokeRect(sx+1,sy+1,CELL-2,CELL-2);
    ctx.strokeStyle='rgba(160,160,220,0.3)';ctx.lineWidth=1;
    ctx.beginPath();ctx.moveTo(sx+3,sy+3);ctx.lineTo(sx+CELL-3,sy+CELL-3);ctx.stroke();
    ctx.beginPath();ctx.moveTo(sx+CELL-3,sy+3);ctx.lineTo(sx+3,sy+CELL-3);ctx.stroke();
    ctx.restore();
  });
  // Encounter range ring
  ctx.save();ctx.strokeStyle='rgba(255,255,255,.15)';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.arc(W/2,H/2,ENCOUNTER_RANGE*CELL+CELL/2,0,Math.PI*2);ctx.stroke();ctx.restore();
  // FIX 2 — sprites isolated with save/restore
  GS.spawns.forEach(s=>{
    const relX=s.wx-GS.map.x,relY=s.wy-GS.map.y;
    const visW=Math.ceil(W/(2*CELL))+1;const visH=Math.ceil(H/(2*CELL))+1;
  ...
  const coordsEl=document.getElementById('radar-coords');if(coordsEl)coordsEl.textContent='X:'+GS.map.x+' Y:'+GS.map.y+'  ·  '+MAPS[GS.map.id].name;
}

function renderSpawnList(){
  const el=document.getElementById('spawn-list');if(!el)return;
  const badge=document.getElementById('spawn-count-badge');if(badge)badge.textContent=GS.spawns.length||'';
  if(!GS.spawns.length){el.innerHTML='<div class="no-spawn-ov">Explora el mapa...</div>';return;}
  el.innerHTML=GS.spawns.map(s=>{
    ...
    const sprUrl=s.variant==='shiny'?b.shSp:b.sp;
    const lvR=b.lvR||[3,18];
    return`<div class="spawn-item-ov ${s.variant==='shiny'?'spawn-shiny':s.variant==='dark'?'spawn-dark':''} ${near?'spawn-near':''}" onclick="onSpawnListClick('${s.uid}')">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-size:11px;font-weight:700;display:flex;align-items:center;gap:3px;flex-wrap:wrap">${variantLabel(s.variant)}${variantName(b.n,s.variant)}${genderTag(s.gender)}<span style="color:var(--dim);font-size:9px">Lv.${s.lv}</span></div>
        <div style="display:flex;gap:3px;align-items:center;flex-wrap:wrap;margin-top:1px">${typeBadges(b.t)}<span style="font-size:9px;color:var(--dim)">Nv.${lvR[0]}-${lvR[1]}</span>${near?'<span style="color:#39ff14;font-size:9px;font-weight:700">CERCA</span>':'<span style="font-size:9px;color:var(--dim)">'+d+'cel</span>'}</div>
        <div style="height:2px;background:rgba(255,255,255,.1);border-radius:1px;margin-top:3px"><div style="height:100%;width:${pct}%;background:${timerCol};border-radius:1px;transition:width .5s"></div></div>
      </div>
    </div>`;
  }).join('');
}
function toggleSpawnOv(){
  const body=document.getElementById('spawn-list');
  const chevron=document.getElementById('spawn-ov-chevron');
  if(!body)return;
...
function setupCanvasClick(){
  ...
  cv.onclick=function(e){
    ...
    const px=e.clientX-rect.left-cv.width/2;const py=e.clientY-rect.top-cv.height/2;
    const relX=Math.round(px/CELL);const relY=Math.round(py/CELL);
    const wx=GS.map.x+relX;const wy=GS.map.y+relY;
...
function battleBag(){
  ...
  showOv('enc-ov',`<div class="obox"><div class="otitle"><span class="ms" style="font-size:16px">backpack</span> Mochila</div>
    ${healIs.length?healIs.map(id=>{const it=ITEMS[id];const isRev=it.cat==='revive';return`<div class="heal-pick-item" onclick="battlePickItem(${id})">
...
function renderBattle(){
  ...
  const eDark=e.variant==='dark'?'filter:grayscale(1) brightness(0.28)':'';
  const pDark=p.variant==='dark'?'filter:grayscale(1) brightness(0.28)':'';
  // FIX 4 — transform tag
  ...
  const mapCol=MAPS[GS.map.id]?.col||'#3498db';
  sc.innerHTML=`<div class="battle-wrap" style="background:linear-gradient(to bottom,${mapCol}22,${mapCol}06,transparent)">
...
function goTo(screen){
  if(GS.battle&&!GS.battle.over&&screen!=='battle'){notif('No disponible en batalla','err',2500);return;}
  GS.screen=screen;
...
function updateHUD(){
  ...
  const hudImg=document.getElementById('hud-avatar-img');const hudPh=document.getElementById('hud-avatar-ph');
  const av=GS.player.avatar||'';
  if(hudImg){hudImg.src=av||DEFAULT_AVATAR;hudImg.style.display='block';}
  if(hudPh)hudPh.style.display=av?'none':'flex';
  const inBattle=!!(GS.battle&&!GS.battle.over);
  document.querySelectorAll('.nav-btn').forEach(b=>{b.classList.toggle('nav-battle-lock',inBattle);});
  ...
  t.innerHTML=GS.player.team.map(p=>{
    ...
    const imgSt=p.currentHp<=0?'filter:grayscale(1) opacity(.3)':p.variant==='dark'?'filter:grayscale(1) brightness(.35)':'';
    return`<div class="hud-slot ${vClass}" title="${p.nickname||p.name} Lv.${p.lv}${p.currentHp<=0?' [KO]':''}${p.variant!=='normal'?' ['+p.variant.toUpperCase()+']':''}">
      <img src="${getSprite(p)}" style="${imgSt}" onerror="this.style.display='none'"></div>`;
  ...
  if(av){loadSpr(av,()=>{if(GS.screen==='radar')renderRadarCanvas();});}
}
function updateAllAvatars(){
  const av=GS.player.avatar||DEFAULT_AVATAR;
  delete _sprCache[av];
  loadSpr(av,()=>{updateHUD();renderRadarCanvas();});
  const heroImg=document.getElementById('profile-hero-img');if(heroImg)heroImg.src=av;
}
...
// FIX 6 — fainted cards: only revive visible; no heal when HP=0
function pkmCard(pkm,idx,src){
  ...
  const nickDisplay=pkm.nickname?`<span class="vname-${pkm.variant==='shiny'?'shiny':pkm.variant==='dark'?'dark':''}" style="${pkm.variant==='normal'?'':''}">${pkm.nickname}</span><span class="pkm-species">${pkm.name}</span>`:pkm.variant==='shiny'?`<span class="vname-shiny">${pkm.name}</span>`:pkm.variant==='dark'?`<span class="vname-dark">${pkm.name}</span>`:pkm.name;
  const srcIcon=pkm.captureSource==='code'?'<span class="ms" style="font-size:12px;color:var(--gold)">card_giftcard</span>':pkm.captureSource==='starter'?'<span class="ms" style="font-size:12px;color:var(--blue)">catching_pokemon</span>':'<span class="ms" style="font-size:12px;color:var(--dim)">filter_drama</span>';
  return`<div class="pkm-card ${isFainted?'fainted':''} ${cardCls}">
    <div class="pkm-card-hdr">
      ...
      <div class="pkm-title">
        <div class="pkm-name" style="display:flex;align-items:center;gap:4px;flex-wrap:wrap">${variantLabel(pkm.variant)} ${nickDisplay} ${genderTag(pkm.gender)}</div>
        <div class="pkm-lv">Lv.${pkm.lv}${pkm.lv>=100?' <span style="color:var(--gold);font-size:9px">MAX</span>':''}</div>
        <div class="pkm-tags" style="display:flex;align-items:center;gap:4px;flex-wrap:wrap">
    ...
    :`<div class="statrow"><span style="min-width:28px;color:var(--dim)">HP</span><div class="statbar"><div class="statbar-fill" style="width:${pp}%;background:${hpCol(pp)}"></div></div><span>${pkm.currentHp}/${pkm.maxHp}</span></div>`}
    <div class="card-acts">
      ...
      ${pkm.status?`<button class="cact" style="background:rgba(255,100,100,.1);color:#ff8888;border:1px solid rgba(255,100,100,.3)" onclick="cureStatus('${pkm.uid}','${src}',${idx})"><span class="ms" style="font-size:14px">medication</span>Curar ${STATUS_TAG[pkm.status]||''}</button>`:''}
...
function openRename(uid){
  ...
  showOv('enc-ov',`<div class="obox">
    <div class="otitle"><span class="ms" style="font-size:16px;color:var(--blue)">edit</span> Apodo para ${pkm.name}</div>
    <input id="rename-in" class="prof-in" value="${pkm.nickname||''}" placeholder="${pkm.name}" maxlength="20" style="width:100%"
      onkeydown="validateAlNum(event);stopMapKeys(event);if(event.key==='Enter')doRename('${uid}')"
      onfocus="stopMapKeys(event)" oninput="stopMapKeys(event)">
    <div style="display:flex;gap:8px;width:100%">
      <button class="ebtn ebtn-catch" style="flex:1" onclick="doRename('${uid}')"><span class="ms">check</span>Guardar</button>
      <button class="ebtn" style="background:rgba(231,76,60,.1);color:var(--acc);border:1px solid rgba(231,76,60,.3)" onclick="doRename('${uid}',true)"><span class="ms">clear</span>Quitar</button>
...
function doRename(uid,clear=false){
  const pkm=[...GS.player.team,...GS.player.pc].find(p=>p.uid===uid);if(!pkm)return;
  if(clear){pkm.nickname=null;}else{const el=document.getElementById('rename-in');if(el)pkm.nickname=el.value.trim().slice(0,20)||null;}
...
function renderPC(){
  ...
  <div class="pc-controls">
    <div class="pc-search-wrap">
      <span class="ms" style="color:var(--dim);font-size:18px">search</span>
      <input id="pc-search-in" class="pc-search-in" type="text" placeholder="Buscar nombre, tipo, rareza..."
        onkeydown="stopMapKeys(event)" onfocus="stopMapKeys(event)"
        oninput="GS.pcSearch=this.value;GS.pcPage=0;renderPCList()">
      <button id="pc-clear-btn" class="pc-clear-search" style="display:none" onclick="GS.pcSearch='';GS.pcPage=0;document.getElementById('pc-search-in').value='';document.getElementById('pc-clear-btn').style.display='none';renderPCList()"><span class="ms" style="font-size:16px">close</span></button>
    </div>
    <div class="view-mode-bar" style="margin:0">
      <button class="vm-btn ${GS.viewMode==='detailed'?'active':''}" onclick="GS.viewMode='detailed';save();renderPCList()"><span class="ms" style="font-size:14px">dashboard</span>Det.</button>
      <button class="vm-btn ${GS.viewMode==='simple'?'active':''}" onclick="GS.viewMode='simple';save();renderPCList()"><span class="ms" style="font-size:14px">view_list</span>Simple</button>
    </div>
  </div>
  <div id="pc-list-wrap"></div>
  <div id="pc-pages"></div>`;
  if(GS.pcSearch){const si=document.getElementById('pc-search-in');if(si)si.value=GS.pcSearch;const cb=document.getElementById('pc-clear-btn');if(cb)cb.style.display='flex';}
  renderPCList();
}
function renderPCList(){
  const wrap=document.getElementById('pc-list-wrap');const pages=document.getElementById('pc-pages');if(!wrap)return;
  ...
  const cb=document.getElementById('pc-clear-btn');if(cb)cb.style.display=GS.pcSearch?'flex':'none';
  ...
  if(pages)pages.innerHTML=total>1?`<div class="pc-pagination"><button class="page-btn" onclick="GS.pcPage=${Math.max(0,cp-1)};renderPCList()" ${cp===0?'disabled':''}><span class="ms">chevron_left</span></button><span class="page-info">${cp+1}/${total}</span><button class="page-btn" onclick="GS.pcPage=${Math.min(total-1,cp+1)};renderPCList()" ${cp===total-1?'disabled':''}><span class="ms">chevron_right</span></button></div>`:'';
}
...
function renderShop(){
  ...
  const cats=[['pokeball','Poké Balls','catching_pokemon'],['candy','Caramelos Raros','star'],['heal','Curación y Estado','medication'],['revive','Revivir','healing'],['evo','Evolución','auto_awesome'],['held','Held Items','backpack'],['mt','MTs','library_add']];
    ...
    return`<div class="shop-cat"><h3><span class="ms" style="font-size:16px;color:#f39c12">${icon}</span> ${label}</h3><div class="shop-list">
      ${its.map(it=>`<div class="shop-item">
        ...
        <div class="shop-actions"><span class="shop-price">${it.price>=99999?'∞':it.price.toLocaleString()}G</span>
          ${it.id===200&&GS.player.expShareOn?`<span style="font-size:10px;color:var(--green);font-weight:700">✓ Activo</span>`:it.id===200&&cntBag(it.id)>=1?`<span style="font-size:10px;color:var(--dim)">Ya tienes uno</span>`:it.price<99999?`<button class="btn-buy" onclick="buyItem(${it.id})">Comprar</button>`:''}
...
function renderInventory(){
  ...
  ${bag.length?`<div class="inv-grid">${bag.map(it=>`<div class="inv-item">
    ...
    <div class="inv-item-acts">
      ...
      ${it.cat==='held'&&it.id===200?(GS.player.expShareOn?`<button class="inv-use-btn" style="background:rgba(46,204,113,.15);color:var(--green);border-color:rgba(46,204,113,.3);cursor:not-allowed" disabled>✓ Activado</button>`:`<button class="inv-use-btn" onclick="activateHeld(${it.id})">Activar</button>`):it.cat==='held'?`<button class="inv-use-btn" onclick="activateHeld(${it.id})">Activar</button>`:''}
...
function useHealFromInv(id){
  ...
  showSearchablePicker('Usar '+it.n,targets.map(p=>({sp:getSprite(p),name:(p.nickname||p.name),sub:p.currentHp+'/'+p.maxHp+' HP',onclick:"doUseHealOnPkm('"+p.uid+"',"+id+")"})));
}
function useReviveFromInv(id){
  ...
  showSearchablePicker('Revivir con '+it.n,targets.map(p=>({sp:getSprite(p),name:(p.nickname||p.name),sub:'0/'+p.maxHp+' HP — Debilitado',onclick:"doUseReviveOnPkm('"+p.uid+"',"+id+")"})));
}
...
function doUseEvoOnPkm(uid,itemId){
  closeOv('enc-ov');
  const pkm=[...GS.player.team,...GS.player.pc].find(p=>p.uid===uid);if(!pkm)return;
  const it=ITEMS[+itemId];const b=PD[pkm.pid];
  if(!it||!b||!b.evo||!b.evo.length){notif('Error al evolucionar.','err');return;}
  if(!cntBag(itemId)){notif('Sin ese ítem.','err');return;}
  pkm.evoPending=b.evo[0];pkm._pendingEvoItemId=+itemId;
  save();showEvoPrompt(pkm);
}
...
function activateHeld(id){
  if(+id===200){
    if(GS.player.expShareOn){notif('Ya está activo','warn');return;}
    GS.player.expShareOn=true;notif('¡Repartir Exp activado!','ok');
  }
...
function renderProfile(){
  ...
  const badgesHTML=Object.entries(TRAINERS).map(([r,rd])=>{
    ...
    return`<div class="badges-region">
      <div class="badges-region-title" style="color:${rd.col}">${rd.name} <span style="font-size:10px;color:var(--dim)">${cnt}/${rd.trainers.length}</span></div>
      <div class="badges-flex">${rd.trainers.map(t=>{const won=t.def||GS.player.defeatedTrainers.has(t.id);
        return`<div class="badge-cell ${won?'badge-earned':'badge-locked'}">
          <div class="badge-icon" style="${won?'background:'+t.medalCol+';box-shadow:0 0 10px '+t.medalCol+'66':'background:var(--card2)'}">${t.medalIcon||'🏅'}</div>
          <span class="badge-name" style="color:${won?t.medalCol:'var(--dim)'}">${t.medal||'Medalla'}</span>
          <span style="font-size:9px;color:var(--dim);text-align:center">${t.n}</span>
        </div>`;}).join('')}</div></div>`;}).join('');
  sc.innerHTML=`
  <div class="profile-hero">
    ...
    <div class="profile-hero-overlay">
      ...
      <div style="display:flex;gap:6px">
        <button class="prof-hero-btn" onclick="openEditNamePopup()" title="Cambiar nombre"><span class="ms" style="font-size:16px">edit</span></button>
        <button class="prof-hero-btn" onclick="openEditAvatarPopup()" title="Cambiar foto"><span class="ms" style="font-size:16px">add_a_photo</span></button>
      </div>
    </div>
  </div>
  <div style="padding:12px;display:flex;flex-direction:column;gap:12px">
    <div class="prof-stats">
      ...
      <div class="pstat"><span>Entrenadores</span><strong>${defCnt}</strong></div>
    ...
    <div class="export-section">
      <h3><span class="ms" style="font-size:16px">save</span> Partida</h3>
...
function openEditAvatarPopup(){
  ...
  showOv('enc-ov',`<div class="obox">
    ...
    <img id="pop-av-prev" src="${av||DEFAULT_AVATAR}" style="width:80px;height:80px;border-radius:50%;border:3px solid var(--blue);object-fit:cover;image-rendering:pixelated" onerror="this.src='${DEFAULT_AVATAR}'">
    <input id="pop-av-in" class="prof-in" value="${av}" placeholder="URL de imagen (opcional)" style="width:100%"
      onkeydown="stopMapKeys(event)" onfocus="stopMapKeys(event)"
      oninput="var i=document.getElementById('pop-av-prev');if(i)i.src=this.value||'${DEFAULT_AVATAR}'">
    <div style="display:flex;gap:8px;width:100%">
      <button class="ebtn ebtn-catch" style="flex:1" onclick="saveAvatarPopup()"><span class="ms">check</span>Guardar</button>
  ...
  setTimeout(()=>{const i=document.getElementById('pop-av-in');if(i)i.focus();},60);
}
function saveAvatarPopup(){
  const i=document.getElementById('pop-av-in');if(!i)return;
...
function openEditNamePopup(){
  showOv('enc-ov',`<div class="obox">
    <div class="otitle"><span class="ms" style="font-size:16px;color:var(--blue)">edit</span> Cambiar nombre</div>
    <input id="pop-name-in" class="prof-in" value="${GS.player.name}" maxlength="20" style="width:100%"
      onkeydown="validateAlNum(event);stopMapKeys(event)" onfocus="stopMapKeys(event)" oninput="stopMapKeys(event)">
    <div style="display:flex;gap:8px;width:100%">
      <button class="ebtn ebtn-catch" style="flex:1" onclick="saveNamePopup()"><span class="ms">check</span>Guardar</button>
      <button class="ebtn ebtn-flee" onclick="closeOv('enc-ov')"><span class="ms">close</span></button>
    </div>
  </div>`);
  setTimeout(()=>{const i=document.getElementById('pop-name-in');if(i)i.focus();},60);
}
function saveNamePopup(){
  const i=document.getElementById('pop-name-in');if(!i)return;
  const n=i.value.trim();if(n)GS.player.name=n;save();closeOv('enc-ov');notif('Nombre actualizado','ok');renderProfile();
}
function exportSave(){const data=localStorage.getItem('pf3_save');if(!data){notif('No hay partida.','warn');return;}const blob=new Blob([data],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='pokem_'+Date.now()+'.json';a.click();URL.revokeObjectURL(url);notif('Partida exportada','ok');}
function importSave(){const inp=document.createElement('input');inp.type='file';inp.accept='.json';inp.onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>{try{JSON.parse(ev.target.result);if(!confirm('¿Sobrescribir partida actual?'))return;localStorage.setItem('pf3_save',ev.target.result);notif('¡Importada!','ok',2000);setTimeout(()=>location.reload(),1500);}catch(e){notif('Archivo inválido','err');}};r.readAsText(f);};inp.click();}
...
function saveAvatar(){
  ...
  notif('Foto de perfil actualizada.','ok');
}
...
function openMapInfo(){
  ...
  const poolHTML=m.pool.map(pid=>{const b=PD[pid];const lvR=b.lvR||[3,18];
    return`<div class="map-info-pkm"><img src="${b.sp}" style="width:38px;height:38px;image-rendering:pixelated;object-fit:contain" onerror="this.style.display='none'">
      ...
      <div>${typeBadges(b.t)}</div></div></div>`;}).join('');
  ...
  if(isNew){
    ov.innerHTML=`<div class="intro-box">
      ...
      <div style="width:100%;display:flex;flex-direction:column;gap:10px;margin-top:4px">
        ...
        <input id="intro-avatar" class="prof-in" placeholder="URL de imagen (opcional)" style="width:100%" onkeydown="stopMapKeys(event)" onfocus="stopMapKeys(event)" oninput="var i=document.getElementById('intro-av-preview');if(i)i.src=this.value||'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/versions/generation-v/black-white/animated/25.gif'">
      </div>
      <div style="font-size:12px;color:var(--dim);margin-top:10px;text-align:center">¿Con quién empiezas?</div>
      <div id="intro-av-preview-wrap" style="display:flex;justify-content:center;margin:2px 0">
          <img id="intro-av-preview" src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/versions/generation-v/black-white/animated/25.gif"
            style="width:60px;height:60px;border-radius:50%;border:2px solid var(--blue);object-fit:cover;image-rendering:pixelated" onerror="this.style.display='none'">
        </div>
      <div id="starter-err" style="font-size:10px;color:var(--acc);display:none;text-align:center">⚠ Selecciona un Pokémon inicial</div>
...
function selectStarter(pid){
  ...
  const sg=document.getElementById('starter-err');if(sg)sg.style.display='none';
}
function startAdventure(){
  const nameEl=document.getElementById('intro-name');
  const name=(nameEl?.value||'').trim();
  let err=false;
  if(!name){if(nameEl)nameEl.classList.add('field-error');notif('Escribe tu nombre de entrenador','err');err=true;}
  if(!_selectedStarterId){const sg=document.getElementById('starter-err');if(sg)sg.style.display='block';notif('Elige un Pokémon inicial','err');err=true;}
  if(err)return;
  ...
  if(starter){starter.captureSource='starter';GS.player.team.push(starter);}
  addBag(1,5);addBag(10,3);
  save();closeIntro();
  notif('¡'+GS.player.name+', comienza tu aventura!','ok',4000);
}
function hudAvatarClick(){if(GS.screen==='profile')goTo('radar');else goTo('profile');}
...
window.addEventListener('DOMContentLoaded',()=>{
  load();
  showIntro();
  // Preload default avatar always
  loadSpr(DEFAULT_AVATAR,()=>{updateHUD();renderRadarCanvas();});
  if(GS.player.avatar&&GS.player.avatar!==DEFAULT_AVATAR)loadSpr(GS.player.avatar,()=>{updateHUD();renderRadarCanvas();});
  updateHUD();setupCanvasClick();
  // Resize canvas to fill container
  function doResize(){resizeRadarCanvas();renderRadarCanvas();}
